module.exports = {
  ...
  onStart: async function ({ args, event, api, usersData }) {
    const permission = ["61563485781052","61561568196014",];
  if (!permission.includes(event.senderID)) {
    api.sendMessage("You don't have enough permission to use this command. Only Xue Editz can do it.", event.threadID, event.messageID);
    return;
  }
    const query = args[0];
